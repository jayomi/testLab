...
<body
	...
	<div class="page-container">
		...
		<div class="page-content-wrapper">
				...
				<!-- BEGIN PAGE BASE CONTENT -->
						...
						<div class="portlet-body">
                                    ...
                                    <table id="user" class="table table-bordered table-striped">
                                        <tbody>
                                        	 <tr>
                                                ...
                                                <td style="width:50%">
                                                    <a href="javascript:;" id="username" data-type="text" data-pk="1" data-original-title="Enter username">${headToken.tokenStatus } </a>
                                                </td>
                            ...
                            <h1><div>${reportStatus}</div></h1>                            
                      		...
                      		<fieldset>
                      			 <legend>Doctor Statement:</legend>
                      			 	<c:if test="${!empty doctorComment}">
                      			 	<label>${doctorComment} </label>
                      			 	</c:if>   
                      		</fieldset>
			...
			action="/AZHealthPortal/token/closedToken/${headTokenId }" modelAttribute="headToken">
			 --%>
			      
       <form:form class="form-horizontal" method="post" 
			action="/AZHealthPortal/assigToSRP/tokenAssignToSRP/${headTokenId }" modelAttribute="headToken">
                                    ...
                                    <table id="user" class="table table-bordered table-striped">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td style="width:50%">
                                                    <form:textarea path="adminComment" type="text" placeholder="Your Comment" class="form-control"></form:textarea>
                                                </td>                                               
                                            </tr>                                  
                                            ...
                                            <tr>
                                                <td> Change Status:  </td>
                                                <td>                                                
	                                                    <form:select path="tokenStatus" id="assignType">
	                                                        <option>Select</option>
	                                                        <form:option value="toSRP">Assign To SRP </form:option>
	                                                        <form:option value="closed">closed token </form:option>	                                                                                                              
	                                                    </form:select>	                                                	
                                                </td>
                                                
                                            </tr> 
                                            
                                           <tr id="selectSRP" hidden="hidden">
                                            <td>
                                            	Select SRP: 
                                            </td>
                                            <td>
                                            	<c:if test="${!empty srpList}">
													<form:select path="srpId" id="srpId" class="form-control"> <!-- doctorName -->
														<%-- <form:option value="no"> - Select - </form:option> --%>
														<c:forEach var="i" items="${srpList}" varStatus="status">																
														
														<form:option value="${i.id}">${i.labName} , ${i.fullName}</form:option>																				
                                            ...
                                            <c:forEach var="i" items="${fileNameList}" varStatus="status">																
												<tr class="odd gradeX">
												 <td>
                                                  ...
                                                  <form:checkbox path="assignToSrpReport" class="checkReport checkboxes" value="${i}" />
                                                   <%-- <input type="checkbox" path="" name="" class="checkReport checkboxes" value="${i}"  /> --%>
                                                 </td>                                                	
                                                	<td> <c:out value="${i}" /> </td>                                	
 ...
 <!-- BEGIN SCRIPT -->
    ...
    <script type="text/javascript">
	  ...
	  $(document).on('change', '#assignType', function () {
			...
			if(option == "toSRP"){//doctor
				//alert(option);
				$('#selectSRP').show();
				$('#selectReport').show();				
				$("#srpId").attr("required","required");
				//$("#selectSRP").removeAttr("hidden");
				
			}else if(option == "toSRP"){
				$('#selectSRP').hide();
				$('#selectReport').hide();
				$("#srpId").removeAttr("required");
